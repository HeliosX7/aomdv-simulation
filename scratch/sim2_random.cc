...
EnergyCheck(NodeContainer c) {
  int x = 0;
    for(NodeContainer::Iterator i = c.Begin() ; i < c.End() ; i++, x++){
        double e = ((*i)->GetObject<EnergySourceContainer>()->Get(0)->GetRemainingEnergy());
        Emin = std::min(Emin, e);
        NS_LOG_UNCOND("energy of node finally, "<< x << " = " << e);
        remainingEnergy += e;
    }
...
main (int argc, char *argv[])
{
  ...
  uint32_t sourceNode=0 , sourceNode2 = 1;
  ...
  */
  ...
  uint32_t minInitialEnergy = 9000;
  uint32_t maxInitialEnergy = 12000;  
  ...
  for(auto pp : NodeInitialEnergyMap){
        NS_LOG_UNCOND("energy of node at start, "<< pp.first << " = " << pp.second);
  ...
  Ptr<Socket> recvSink2 = Socket::CreateSocket (c.Get (sinkNode2), tid2);
  ...
  InetSocketAddress remote2 = InetSocketAddress (i.GetAddress (sinkNode2, 0), 80);
  ...
  double stopTime = 2200.0;
  Simulator::Schedule (Seconds (5.0), &GenerateTraffic, source, packetSize, numPackets,
                       interPacketInterval);
  Simulator::Schedule (Seconds (1100.0), &GenerateTraffic, source2, packetSize, numPackets,
                       interPacketInterval);
